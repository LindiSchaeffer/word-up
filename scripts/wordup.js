...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        // TODO 13 what should the url be? DONE
...
function render() {
    ...
    var wordGuess = model.wordSubmissions.map(wordSubmissionChip)
    $("#word-submissions").append(wordGuess);
...
$(document).ready(function() {
    ...
    $("#textbox").on("input", function() {
        model.currentAttempt = $("input:text").val();
        render();
    });
...
function isDisallowedLetter(letter) {
    ...
    var n = model.allowedLetters.indexOf(letter);
    if (n == -1)
        return true
    else
        return false;
